...
class PPOAgent(BaseAgent):
    def __init__(self, config=None):
        ...
        self.device = self.train_device  # ""cuda" if torch.cuda.is_available() else "cpu"
    ...
    def compute_returns(self):
        returns = []
        with torch.no_grad():
            _, values = self.policy(self.states) # forward()?
    ...
    def get_action(self, observation, evaluation=False):
        ...
        distribution, _ = self.policy.forward(state)
        # print(distribution.mean, distribution.covariance_matrix)
        if evaluation:
